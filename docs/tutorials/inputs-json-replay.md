...
- **Match `--endpoint-type` to your payload format** (e.g., `chat` for chat completions, `embeddings` for embeddings). This gives you correct response parsing, endpoint path handling, and metrics. Use `--endpoint-type raw` as a fallback for non-standard APIs -- it skips path appending (so you must include the full path in `--url`) and uses auto-detection for response parsing.
